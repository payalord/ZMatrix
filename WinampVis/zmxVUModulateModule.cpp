...
//
//  Program:   vis_zmx
//  Module:    $RCSfile: zmxVUModulateModule.cpp,v $
//  Language:  C/C++
...
#define MODULE_NAME "VU Modulate"
...
float BaseOffsets[3] = {0.0f,64.0f,128.0f};
float PeakScales[3] = {2.0f,2.0f,2.0f};
float PeakOffsets[3] = {128.0f,255.0f,255.0f};
float GlobalScale = 3.0f;
float GlobalOffset = -0.3f;
...
int render(struct winampVisModule *this_mod)
{
	...
	{
        float Ch1NormFactor = 1.0;
        float Ch2NormFactor = 1.0;
        ...
        for (y = 0; y < 2; y ++)
        {
            ...
            if(y)
            {
                Ch1NormFactor = (float)total/255;
            }
            else
            {
                Ch2NormFactor = (float)total/255;
            }
        }
	/*
		int x, y;
		...
		for (y = 0; y < 2; y ++)
		{
			...
			for (x = 1; x < 576; x ++)
			{
				total += this_mod->waveformData[y][x];
				last = this_mod->waveformData[y][x];
			}
		}
		total /= 2*576;
		...
		FadeB = 0;
*/
        float NormFactor = (Ch1NormFactor + Ch2NormFactor)/2.0;
