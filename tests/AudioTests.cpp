...
int wmain() {
    ...
    try {
        ...
        Check(!settings.enabled && settings.mode == audio::LegacyVU && audio::Valid(settings),"Audio must default to disabled.");
        const auto base = audio::Map(settings.profiles[0],0), peak = audio::Map(settings.profiles[0],1);
        Check(base.scale[0] == 0 && base.offset[1] == 64 && base.offset[2] == 128,"VU base colors changed.");
        Check(peak.scale[2] == 2 && peak.offset[0] == 128 && peak.offset[1] == 255,"VU peak colors changed.");
        ...
        Check(audio::Load(file.c_str(),loaded) == 0 && loaded.enabled && loaded.mode == audio::Frequency &&
        ...
        Check(audio::LegacyVariation(wave) == 0,"Constant waveform was not silent.");
        for(unsigned i = 0; i < 576; ++i) wave[i] = i%2 ? 255 : 0;
        Check(audio::LegacyVariation(wave) == 1,"Legacy VU saturation changed.");
        wave[0] = 255; std::fill(wave+1,wave+576,static_cast<unsigned char>(0));
        Check(audio::LegacyVariation(wave) == 0,"Legacy VU integer division changed.");
        for(unsigned rate : {22050u,44100u,48000u,96000u,192000u}) {
            for(unsigned channels : {1u,2u,6u}) {
                ...
                Check(Near(low.frequency,1000.0/11025,0.002) && Near(high.frequency,4000.0/11025,0.002),"Frequency centroid or sample-rate compensation failed.");
                Check(high.vu > low.vu,"Legacy VU lost its frequency sensitivity.");
                const auto quiet = Tone(rate,channels,1000,0.05,true);
                Check(Near(quiet.frequency,low.frequency,1e-5),"Frequency centroid depends on amplitude.");
            }
        }
        ...
        Check(analyzer.Analyze().frequency == 0,"Nonfinite PCM reached the FFT.");
        analyzer.Push(nullptr,8192,true);
        Check(analyzer.Analyze().frequency == 0 && analyzer.Analyze().vu == 0,"WASAPI silent packets were not handled.");
